...
function clean_text($text) {
    ...
    // Normalize smart/curly quotes and other UTF-8 punctuation to ASCII equivalents
    // Using actual UTF-8 encoded characters for replacement
    $text = str_replace([
        "\xe2\x80\x9c", "\xe2\x80\x9d", "\xe2\x80\x98", "\xe2\x80\x99", // " " ' '
        "\xe2\x80\xba", "\xe2\x80\x9e", "\xe2\x80\x9f",             // ‚ „ ‟
        "\xe2\x80\x93", "\xe2\x80\x94", "\xe2\x80\x95",             // en-dash, em-dash, horiz-bar
        "\xe2\x80\xa6",                                      // ellipsis
        "\xc2\xb4", "\xc2\xb8", "\xc2\xb9",                     // accents
    ], [
        '"', '"', "'", "'",
        "'", '"', '"',
        '-', '-', '-',
...
// Truncate a UTF-8 string to a given byte length, appending '...' if truncated.
// Uses mb_strcut so multi-byte characters are never split in half.
function truncate_str($text, $max_bytes) {
    if (mb_strlen($text, 'UTF-8') * 1 <= $max_bytes || mb_strcut($text, 0, $max_bytes, 'UTF-8') === $text) {
        return $text;
    }
...
function create_ascii_table($appuntamenti) {
    ...
    foreach ($appuntamenti as $app) {
        $luogo = clean_text($app['luogo']);
        $luogo = truncate_str($luogo, $LUOGO_WIDTH);
        ...
        // The title row is prefixed with "-> " (3 chars), so available space is LUOGO_WIDTH - 3
        $title_max = $LUOGO_WIDTH - 3;
        $titolo_visualizzato = truncate_str($titolo, $title_max);
...
function create_ascii_list_table($appuntamenti) {
    ...
    // Data rows
    foreach ($appuntamenti as $app) {
        $corso_raw   = clean_text($app['corso']);
        $corso       = truncate_str($corso_raw,          $col_progetto);
        $luogo_raw   = clean_text($app['luogo']);
        $luogo       = truncate_str($luogo_raw,          $col_luogo);
        $oraInizio   = truncate_str($app['oraInizio'],   $col_inizio);
        $oraFine     = truncate_str($app['oraFine'],     $col_fine);
        $desc_raw    = clean_text($app['descrizione']);
        $descrizione = truncate_str($desc_raw,           $col_descrizione);

        $table .= "|" . str_pad($corso,       $col_progetto, " ", STR_PAD_RIGHT)
                 . "|" . str_pad($luogo,       $col_luogo, " ", STR_PAD_RIGHT)
                 . "|" . str_pad($oraInizio,   $col_inizio, " ", STR_PAD_RIGHT)
                 . "|" . str_pad($oraFine,     $col_fine, " ", STR_PAD_RIGHT)
                 . "|" . str_pad($descrizione, $col_descrizione, " ", STR_PAD_RIGHT) . "|\n";
    }
